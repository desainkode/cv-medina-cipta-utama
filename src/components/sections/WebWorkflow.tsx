...
import { MessageCircle, Palette, Code, RefreshCcw, Rocket } from 'lucide-react';

export function WebWorkflow() {
  const { t } = useLanguage();
            ...
            {steps.map((step, i) => (
              <div key={i} className="flex flex-col items-center text-center group">
                <div className="relative mb-8">
                  <div className="w-20 h-20 rounded-3xl bg-[#111111] border border-white/10 flex items-center justify-center group-hover:border-[#b31b2c] group-hover:scale-110 transition-all duration-500 shadow-xl">
                    <step.icon size={32} className="text-[#b31b2c]" />
                  </div>
                  <div className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-[#b31b2c] text-white flex items-center justify-center font-black text-sm border-4 border-[#0a0a0a] shadow-lg">
                    {i + 1}
                  </div>
                </div>
